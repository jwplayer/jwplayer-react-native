...
| **`onFullScreenExitRequested`**  | Player is about to exit fullscreen. On **iOS** (SDK 4.28.0+) the event carries `reason` (see `FullScreenExitReasonIOS` in index.d.ts); on earlier iOS SDKs and on **Android** no `reason` is sent yet.                      | On **iOS** `{ reason?: string }`, on **Android** `{ message: string }`                                                                                                                                                                                                                                                           |
| **`onFullScreenExit`**           | Player exited fullscreen. Carries `reason` on both platforms in different, non-interchangeable wire formats — camelCase on iOS (SDK 4.28.0+, `FullScreenExitReasonIOS`), hyphenated on Android (`FullScreenExitReasonAndroid`). | On **iOS** `{ reason?: string }`, on **Android** `{ message: string, reason?: string }`                                                                                                                                                                                                                                           |
...
| **`onMeta`**                    | Playback reached timed metadata (ID3, `EXT-X-DATERANGE` incl. SCTE-35, `EXT-X-PROGRAM-DATE-TIME`, DASH emsg, `externalMetadata` cue points) or media / access-log metadata was received. Mirrors the web player's `meta` event. See [Metadata Events](./METADATA-EVENTS.md). | `{metadataType: String, metadataTime?: Double, metadata?: Object, programDateTime?: String, ...}` |
| **`onMetadataCueParsed`**       | A metadata cue was parsed from the manifest / segment ahead of playback reaching it. Mirrors the web player's `metadataCueParsed` event. See [Metadata Events](./METADATA-EVENTS.md).                                                                     | Same shape as `onMeta`                                                                             |
