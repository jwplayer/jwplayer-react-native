...
| **`onFullScreenExitRequested`** | Player is about to exit fullscreen. On iOS the event carries `reason` (see `FullScreenExitReason` in index.d.ts). | `{ reason?: string }` (iOS only) |
| **`onFullScreenExit`**          | Player exited fullscreen. On iOS the event carries `reason` (see `FullScreenExitReason` in index.d.ts). | `{ reason?: string }` (iOS only) |
